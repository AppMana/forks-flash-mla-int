...
def test_sparse_mla_prefill_two_stream_parity(H, T):
    ...
    fp8_ds_mla caches with different block sizes, per-token global slot ids,
    per-token lens, and num_splits==1 direct output.
    """
    if not torch.cuda.is_available() or torch.cuda.get_device_capability(0)[0] != 8:
        pytest.skip("requires Ampere (capability 8.x)")
    from flash_mla import sparse_mla_prefill
    ...
    swa_topk, extra_topk = 384, 256
    scale = 1.0 / math.sqrt(_HEAD_DIM)
    swa_cache, swa_K = _build_cache(swa_topk + 64, 32, dev)
    extra_cache, extra_K = _build_cache(extra_topk + 64, 16, dev)
    swa_slots, extra_slots = swa_K.shape[0], extra_K.shape[0]
